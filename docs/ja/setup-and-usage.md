...
RSSフィード（定性情報）と取引所マーケットデータ（定量情報）を組み合わせ、GitHub Copilot Models API で投資判断を自動化するシステムです。分析結果は構造化JSONとして生成され、確信度が閾値を超えた場合のみ株式・暗号資産取引所へ自動発注します。
...
| コスト最適AI | GitHub Copilot Models API（`GITHUB_MODEL_ID` で設定可能、デフォルト: `openai/gpt-4.1`） |
...
- **GitHub Personal Access Token** — `models:read` スコープ付き。[GitHub Token 設定](https://github.com/settings/tokens) から作成
...
# 3. 環境変数ファイルを用意して GitHub Token 設定を確認
cp .env.example .env
...
# GITHUB_MODEL_ID はデフォルト値があるため、変更する場合のみ設定
...
DynamoDB で冪等性チェック（処理済み記事はスキップ）
    ↓ 新着あり
AI Agent (GitHub Copilot Models API)
...
GitHub Copilot Models API を使用し、単一のモデルで分析を行います。モデルは `GITHUB_MODEL_ID` で設定可能です（デフォルト: `openai/gpt-4.1`）。

| 条件 | 使用モデル | 理由 |
| :--- | :--- | :--- |
| 全分析 | `GITHUB_MODEL_ID` で指定（デフォルト: `openai/gpt-4.1`） | GitHub PAT で認証、高精度な判断 |
...
| `GITHUB_MODEL_ID` | AI モデル ID | `openai/gpt-4.1` | — |
...
### テストで GitHub Token / AI関連エラーが出る

テストは `src/config/env.ts` をモックしているため、実際の GitHub Token は不要です。`GITHUB_MODEL_ID` はデフォルト値を持つため、テスト時に明示的な設定は不要です。`GITHUB_TOKEN` は必須ですがテスト内でモックされます。エラーが出る場合、テストファイル内のモックに新しく追加した環境変数が反映されていない可能性があります。該当テストのモック定義を更新してください。
